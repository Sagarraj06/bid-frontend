...
//   const [formData, setFormData] = useState({
//     sellerName: '',
//     // optional fields intentionally omitted from the UI to avoid sending ""
//     department: '',
//     offeredItem: '',
//     days: 60,
//     limit: 10,
...
//     try {
//       // --- CLEAN THE PAYLOAD: never send empty strings ---
//       const requestPayload = {
//         sellerName: formData.sellerName?.trim(),
//         department: formData.department?.trim() || null,
//         offeredItem: formData.offeredItem?.trim() || null,
//         days: Number(formData.days) || 60,
//         limit: Number(formData.limit) || 10,
//         userId: user.email,
//         filters: {
//           // server may ignore; still send a safe default
...
//       // --- Pick correct API endpoint (fixed https detection) ---
//       const isHttps = window.location.protocol === 'https:';
//       const isLocalhost = ['localhost', '127.0.0.1'].includes(window.location.hostname);

//       let apiUrl: string;
//       if (isHttps && !isLocalhost) {
//         // production over HTTPS -> use your Supabase proxy to avoid mixed-content/CORS
//         const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
//         apiUrl = `${supabaseUrl}/functions/v1/proxy-pdf`;
//         console.log('Using HTTPS proxy API connection:', apiUrl);
//       } else {
//         // localhost or plain HTTP dev -> hit local/edge directly (reverse-proxied in dev)
//         apiUrl = '/api/pdf';
//         console.log('Using direct HTTP API connection:', apiUrl);
//       }
...
//           title: 'Connection Failed',
//           description:
//             'Unable to reach the API server.\n\nCommon causes:\n• HTTPS page calling HTTP API (mixed content)\n• CORS blocked by browser\n\nFix: Serve API over HTTPS or via the provided proxy.',
//           variant: 'destructive',
//         });
//         return;
//       }
...
//           } catch {}
//         }
//         toast({ title: 'Server Error', description: message, variant: 'destructive', duration: 10000 });
//         return;
//       }
...
//         toast({ title: 'Invalid Response Format', description: 'Server returned non-JSON.', variant: 'destructive' });
//         return;
//       }

//       if (!reportData || typeof reportData !== 'object') {
//         toast({ title: 'Invalid Data', description: 'Empty or malformed payload from server.', variant: 'destructive' });
//         return;
//       }

//       // --- Generate PDF (generatePDF returns the jsPDF doc) ---
//       let doc;
//       try {
//         doc = await generatePDF(reportData, { sellerName: formData.sellerName });
//       } catch (pdfErr: any) {
//         console.error('PDF generation failed:', pdfErr);
//         toast({ title: 'PDF Generation Failed', description: pdfErr.message || String(pdfErr), variant: 'destructive' });
//         return;
//       }

//       // --- Save / download ---
//       const fileName = `${(formData.sellerName || 'Seller').replace(/\s+/g, '_')}_Report_${new Date().toISOString().split('T')[0]}.pdf`;
//       try {
//         doc.save(fileName);
//       } catch (err) {
//         console.error('Download failed:', err);
//         toast({ title: 'Download Failed', description: 'PDF was created but could not be downloaded.', variant: 'destructive' });
//         return;
//       }

//       // Persist a small record locally (optional)
//       const id = (window.crypto && 'randomUUID' in window.crypto)
//         ? (window.crypto as any).randomUUID()
//         : Math.random().toString(36).slice(2);
...
//         offeredItem: formData.offeredItem || null,
//         createdAt: new Date().toISOString(),
//         fileName,
//       });
...
//       toast({ title: 'Report Generated Successfully!', description: `Downloaded as ${fileName}` });
//       setTimeout(() => navigate('/dashboard'), 1500);
//     } catch (err: any) {
//       console.error('Unexpected error during report generation:', err);
...
//   const isFormValid = !!formData.sellerName?.trim() && !!formData.email?.trim();
...
//                     max="365"
//                     value={formData.days}
//                     onChange={(e) => setFormData({ ...formData, days: parseInt(e.target.value || '60', 10) })}
//                     className="h-11"
//                   />
//                 </div>
//                 <div className="space-y-2">
...
//                     max="100"
//                     value={formData.limit}
//                     onChange={(e) => setFormData({ ...formData, limit: parseInt(e.target.value || '10', 10) })}
//                     className="h-11"
//                   />
//                 </div>
//               </div>
...
//               <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
//                 <div className="space-y-1">
//                   <p className="text-base font-semibold">Report Generation Cost: 1 Credit</p>
//                   <p className="text-sm text-muted-foreground">
//                     Your current credit balance: <span className="font-semibold text-foreground">{user?.credits || 0} credits</span>
//                   </p>
//                   {user && user.credits < 1 && (
...
// import { generatePDF } from '@/utils/pdfGenerator';  
// import {generatePDF} from '@/utils/pdfGenerator' // ✅
import { generateBidReport } from '@/utils/pdfGenerator';
...
const GenerateReport = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let doc;
      try {
        doc = await generateBidReport(reportData);
      } catch (pdfErr: any) {
        console.error('PDF generation failed:', pdfErr);
        toast({ title: 'PDF Generation Failed', description: pdfErr.message || String(pdfErr), variant: 'destructive' });
        return;
      ...
      const fileName = `${(formData.sellerName || 'Seller').replace(/\s+/g, '_')}_Report_${new Date()
        .toISOString()
        .split('T')[0]}.pdf`;
      try {
        doc.save(fileName);
      ...
      <main className="container mx-auto px-6 py-8 max-w-5xl">
        <form onSubmit={handleSubmit}>
          <Card className="mb-6 border-2">
            ...
            <CardContent className="space-y-6 p-6">
              <div className="space-y-2">
                <Label htmlFor="sellerName" className="text-base font-semibold">
                  Seller/Company Name *
                </Label>
                <Input
                  id="sellerName"
